...
// Relative altitude drifts because this board has no barometer.
...
mod embedded_example {
    ...
    use imu::{
        EskfEstimator, MagnetometerCalibrator, SharedI2c, Vec3, stemma_qt_9dof_accel_gyro_sample,
    ...
    fn main() -> ! {
        ...
        defmt::info!("state log format: t_ms, quaternion, euler_deg, altitude_m");
        defmt::info!(
        ...
        loop {
            ...
            let corrected_mag_mgauss = if magnetometer_enabled {
                match magnetometer.read_magnetic_mgauss() {
                    Ok(mag_mgauss) => {
                        let corrected =
                            mag_calibration.update(stemma_qt_9dof_body_vector(Vec3::new(
                                mag_mgauss.x_mgauss,
                                mag_mgauss.y_mgauss,
                                mag_mgauss.z_mgauss,
                            )));
