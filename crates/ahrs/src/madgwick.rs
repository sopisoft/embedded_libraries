//! Madgwick attitude filter for IMU and MARG data.
...
impl Madgwick {
    ...
    pub const fn with_gradient_gain(gradient_gain: f32) -> Self {
        Self {
            orientation: Quat::IDENTITY,
            gradient_gain: gradient_gain.max(0.0),
        }
    }
    ...
    pub fn set_gradient_gain(&mut self, gradient_gain: f32) {
        self.gradient_gain = gradient_gain.max(0.0);
    }
    ...
    pub fn update_imu(&mut self, gyro_rad_s: Vec3, accel: Vec3, dt: MicrosDurationU32) {
        ...
        if accel != Vec3::ZERO {
            ...
            q_dot1 -= self.gradient_gain * s0;
            q_dot2 -= self.gradient_gain * s1;
            q_dot3 -= self.gradient_gain * s2;
            q_dot4 -= self.gradient_gain * s3;
        }
...
mod tests {
    ...
    fn stationary_update_keeps_identity() {
        let mut filter = Madgwick::with_gradient_gain(0.1);
        filter.update_imu(
            Vec3::ZERO,
