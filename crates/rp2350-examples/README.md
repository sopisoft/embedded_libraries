...
The `rp2350` flight-controller example uses GPIO2–5 for ailerons, elevator, and rudder; GPIO1 for GS-1502; GPIO0 for ESC; GPIO12–13 for CRSF UART; GPIO18–19 for shared I2C; and GPIO6–7 for the status LEDs. Servo power uses an external 5 V BEC with a common ground.
...
| GS-1502 signal wire | Pico 2 GPIO1 (PIO1, physical pin 2) |
...
| LPS25HB SDA | Pico 2 GPIO18 (I2C1 SDA) |
| LPS25HB SCL | Pico 2 GPIO19 (I2C1 SCL) |

The four servo signals are consecutive on the Pico 2 header, with no GND pin between them.

For the `rp2350` flight-controller example, connect the GS-1502 signal to GPIO1 and the ESC signal to GPIO0. Connect CRSF UART to GPIO12 (TX) / GPIO13 (RX), and I2C to GPIO18 (SDA) / GPIO19 (SCL).
