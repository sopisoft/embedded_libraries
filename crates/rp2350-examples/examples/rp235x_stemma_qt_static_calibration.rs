...
mod embedded_example {
    ...
    use imu::{
        AllanImuCalibrator, SharedI2c, StationaryDetection, Vec3, find_i2c_address_by_id,
    ...
    fn is_stationary(
        ...
        let corrected_accel = sample.accel_m_s2.vector() - accel_bias;
        let corrected_gyro = sample.gyro_rad_s.vector() - gyro_bias;
    ...
    fn main() -> ! {
        ...
        while !allan_capture.is_complete() {
            let raw_sample = match accel_gyro.read_accel() {
                Ok(accel_g) => match accel_gyro.read_gyro() {
                    Ok(gyro_dps) => stemma_qt_9dof_accel_gyro_sample(
                        Vec3::new(accel_g.x, accel_g.y, accel_g.z),
                        Vec3::new(gyro_dps.x, gyro_dps.y, gyro_dps.z),
                        GRAVITY_M_S2,
                    ),
                    Err(error) => {
                        defmt::warn!(
                            "gyroscope read failed during capture: {:?}",
                            defmt::Debug2Format(&error)
                        );
                        wait_until(&timer, next_tick);
                        next_tick += period;
                ...
                Err(error) => {
                    defmt::warn!(
                        "accelerometer read failed during capture: {:?}",
                        defmt::Debug2Format(&error)
                    );
                    wait_until(&timer, next_tick);
                    next_tick += period;
            ...
            let corrected_accel = raw_sample.accel_m_s2.vector() - accel_bias;
            let corrected_gyro = raw_sample.gyro_rad_s.vector() - gyro_bias;

            accel_stats.update(corrected_accel - Vec3::new(0.0, 0.0, GRAVITY_M_S2));
        ...
        let allan_capture = match allan_capture.finish() {
            Some(calibration) => calibration,
            None => defmt::panic!("IMU capture did not collect enough samples"),
        };
        ...
        while sample_count < validation_samples {
            ...
            let mag = stemma_qt_9dof_body_vector(Vec3::new(
                mag_mgauss.x_mgauss,
                mag_mgauss.y_mgauss,
                mag_mgauss.z_mgauss,
            ));
