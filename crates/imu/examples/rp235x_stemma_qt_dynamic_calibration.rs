...
// Dynamic calibration helper for RP2350 + Adafruit STEMMA QT LSM6DS3TR-C/LIS3MDL.
//
// Use this while moving the sensor through many orientations to inspect:
// - magnetometer hard-iron offset convergence
// - per-axis span coverage
// - peak gyro / accel excitation during tuning runs
...
mod embedded_example {
    ...
    use imu::{
        AllanImuCalibrator, MagnetometerCalibrator, SharedI2c, Vector3,
        find_i2c_address_by_id, stemma_qt_9dof_accel_gyro_sample, stemma_qt_9dof_body_vector,
    };
    ...
    fn min_component(vec: Vector3) -> f32 {
        vec.x.min(vec.y).min(vec.z)
    }

    fn suggested_mag_calibration_min_span_mgauss(span_mgauss: Vector3) -> f32 {
    ...
    fn main() -> ! {
        ...
        loop {
            ...
            let corrected_mag = mag_calibration.update(stemma_qt_9dof_body_vector(Vector3::new(
                mag_mgauss.x_mgauss,
                mag_mgauss.y_mgauss,
                mag_mgauss.z_mgauss,
            )));
            ...
            if sample_count % (REPORT_PERIOD_SAMPLES * 10) == 0 {
                let span = mag_calibration.span_mgauss();
                let offset = mag_calibration.offset_mgauss();
                defmt::info!(
